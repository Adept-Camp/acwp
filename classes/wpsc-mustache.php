...
class WPSC_Mustache {
    ...
    private function loadTranslationsForTag($tag) {
        ...
        if ($network==77 or $network==100) {
            $native_coin = "xDai";
        } elseif ($network==56 or $network==97) {
            $native_coin = "BNB";
        } elseif ($network==137 or $network==80001) {
            $native_coin = "Matic";
        } else {
            $native_coin = "Ether";
        }
...
TOS;
                ...
                $json = json_encode([
                    'TOS' => $tos,
                    'ARE_YOU_USING_MAINNET' => __('Terms of Service (TOS) - This is BETA Software. Use at your own risk!', 'wp-smart-contracts'),
                    ...
                    'WRONG_RANGE_COMMISSION' => __("Wrong commission", 'wp-smart-contracts'),
                    'WRONG_RANGE_COMMISSION2' => __("Commission should be a value between 0 and 100", 'wp-smart-contracts'),
    ...
    public static function getTemplate($tempid) {
        ...
        if ( !file_exists( $template_path = get_stylesheet_directory() . '/wpsc-views/' . $template ) ) {
            if (!$template_path = self::isNFTThemeTemplate($tempid, $template)) { 
    ...
    static private function isNFTThemeTemplate($tempid, $template) {
        if (array_search($tempid, ["nft-view-all", "nft-my-items", "nft-my-items-card", "nft-view-menu", "nft-author", "nft-collection-mint"])!==false) {
            if ($path = self::getThemePath()) {
                return $path . 'wpsc-views/' . $template;
            }
        }
    }
