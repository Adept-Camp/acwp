...
class WPSC_assets {
    ...
    public function loadAssetsFrontEnd() {
        ...
        if ($is_a_token or $is_a_crowd or $is_a_ico or $is_a_scanner or $is_a_nft) {
    ...
    static public function loadNFTMy($network_name=false, $coin_symbol = false) {
        ...
        if (!isset($coin_symbol) or !$coin_symbol) {
            if (!$collection_id) $collection_id = (int) $_GET["id"];
            if (!$network_id) $network_id = get_post_meta($collection_id, 'wpsc_network', true);
            if (!$network_array) $network_array = WPSC_helpers::getNetworks();
            if ($network_array) {
                $coin_symbol = $network_array[$network_id]["coin-symbol"];
            }
        }
